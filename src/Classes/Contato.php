...
class Contato
{
    ...
    public function criarContato(string $nome, string $telefone){
        ...
        if(is_numeric($telefone) && (strlen($telefone) === 11)){
            //Acrescenta o valor passado para o array como uma nova entrada
            array_push($_SESSION['agenda'], ['id' => $_SESSION['id'], 'Nome'=>$nome, 'Telefone'=>$telefone]);
    ...
    public function editarContato(int $indice, string $novoNome, string $novoTel){
        ...
        if(is_numeric($novoTel) && (strlen($novoTel) === 11)){
            ...
            foreach($_SESSION['agenda'] as $index => $item){
                if($item['id'] === $indice){
                    $_SESSION['agenda'][$index]['Nome'] = $novoNome;
                    $_SESSION['agenda'][$index]['Telefone'] = $novoTel;
                }
            }
        }else{
        ...
        if(!empty($indice) && is_numeric($indice)){
            ...
            foreach ($_SESSION['agenda'] as $index => $item) {
                if ($item['id'] === $indice) {
                    ...
                    //Acrescenta um quando achar o item correspondente
                    $achou++;
                }
            }
